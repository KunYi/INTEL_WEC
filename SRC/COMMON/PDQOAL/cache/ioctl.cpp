                                        ...
                                        ) 
{
    ...
    while(iterationNumber < MAXCPUIIDITERATIONS)
    {
        ...
        for (int i=0; i<4; i++) {
            if (((CPUIDTempBuffer[i] & 0x000000FF) == 0xFF) || ((CPUIDTempBuffer[i] & 0x0000FF00) == 0xFF00) ||
            ((CPUIDTempBuffer[i] & 0x00FF0000) == 0xFF0000) || ((CPUIDTempBuffer[i] & 0xFF000000) == 0xFF000000))
            {
                fCacheInfoByEAX04 = TRUE;
                break;
            }
        }
    ...
    iterationNumber = 0;
    if (fCacheInfoByEAX04)
    {
        UINT32 CPUIDCacheBuffer[4] = {0, 0, 0, 0};
        while (iterationNumber < MAXCPUIIDITERATIONS)
        {
            __try {
                __asm {
                mov eax, 04h
                mov ecx, iterationNumber
                cpuid                         ; when eax is 04h we are querying for cache info
                mov CPUIDCacheBuffer, eax     ; cpuid fills eax, ebx, ecx, and edx with descriptors of cache
                mov CPUIDCacheBuffer+4, ebx
                mov CPUIDCacheBuffer+8, ecx
                mov CPUIDCacheBuffer+12, edx
                }
            }
            __except (GetExceptionCode() == EXCEPTION_ILLEGAL_INSTRUCTION ? EXCEPTION_EXECUTE_HANDLER : EXCEPTION_CONTINUE_SEARCH)
            {
            ...
            if (((CPUIDCacheBuffer[0] & 0xF) == 0x0) ||((CPUIDCacheBuffer[0] & 0xE0) == 0x60)) 
            {
                break;
            }

            if (((CPUIDCacheBuffer[0] & 0xF) == 0x2) &&((CPUIDCacheBuffer[0] & 0xE0) == 0x20)) // L1 Instruction Cache
            {
                g_oalCacheInfo.L1ILineSize |= (CPUIDCacheBuffer[1] & 0xFFF) + 1;
                g_oalCacheInfo.L1INumWays |= ((CPUIDCacheBuffer[1] & 0xFFC00000) >> 22) + 1;
                g_oalCacheInfo.L1ISize |=  g_oalCacheInfo.L1INumWays * (((CPUIDCacheBuffer[1] & 0x3ff000) >> 12) + 1)* 
                    g_oalCacheInfo.L1ILineSize * (CPUIDCacheBuffer[2] +1);
            }
            if (((CPUIDCacheBuffer[0] & 0xF) == 0x1) &&((CPUIDCacheBuffer[0] & 0xE0) == 0x20)) // L1 Data Cache
            {
                g_oalCacheInfo.L1DLineSize |= (CPUIDCacheBuffer[1] & 0xFFF) + 1;
                g_oalCacheInfo.L1DNumWays |= ((CPUIDCacheBuffer[1] & 0xFFC00000) >> 22) + 1;
                g_oalCacheInfo.L1DSize |=  g_oalCacheInfo.L1DNumWays * (((CPUIDCacheBuffer[1] & 0x3ff000) >> 12) + 1)* 
                    g_oalCacheInfo.L1DLineSize * (CPUIDCacheBuffer[2] +1);

            }
            if (((CPUIDCacheBuffer[0] & 0xF) == 0x2) &&((CPUIDCacheBuffer[0] & 0xE0) == 0x40)) // L2 Instruction Cache
            {
                g_oalCacheInfo.L2ILineSize |= (CPUIDCacheBuffer[1] & 0xFFF) + 1;
                g_oalCacheInfo.L2INumWays |= ((CPUIDCacheBuffer[1] & 0xFFC00000) >> 22) + 1;
                g_oalCacheInfo.L2ISize |=  g_oalCacheInfo.L2INumWays * (((CPUIDCacheBuffer[1] & 0x3ff000) >> 12) + 1)*
                    g_oalCacheInfo.L2ILineSize * (CPUIDCacheBuffer[2] +1);
            }
            if (((CPUIDCacheBuffer[0] & 0xF) == 0x3) &&((CPUIDCacheBuffer[0] & 0xE0) == 0x40)) // L2 Data Cache
            {
                g_oalCacheInfo.L2DLineSize |= (CPUIDCacheBuffer[1] & 0xFFF) + 1;
                g_oalCacheInfo.L2DNumWays |= ((CPUIDCacheBuffer[1] & 0xFFC00000) >> 22) + 1;
                g_oalCacheInfo.L2DSize |=  g_oalCacheInfo.L2DNumWays * (((CPUIDCacheBuffer[1] & 0x3ff000) >> 12) + 1)*
                    g_oalCacheInfo.L2DLineSize * (CPUIDCacheBuffer[2] +1);
            }
            iterationNumber++;
        }
    }
