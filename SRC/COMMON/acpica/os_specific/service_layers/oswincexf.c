    ...
    ACPI_SIZE               Length)
{
    ...
    ACPI_PHYSICAL_ADDRESS address  = (ACPI_PHYSICAL_ADDRESS)Where;
    ACPI_PHYSICAL_ADDRESS fromPage = address & ~0x0FFF;    // 4096 page size
    ACPI_SIZE offset               = address &  0x0FFF;
    ACPI_SIZE sizePage             = (((offset + Length) >> 12) + 1) << 12;
    ACPI_PHYSICAL_ADDRESS toPage   = fromPage + sizePage;

	ACPI_FUNCTION_NAME (AcpiOsMapMemory);

	baseVirAddr = NKCreateStaticMapping(fromPage >> 8, sizePage);  // need to shift 8
    virAddr = (void*) ((ACPI_SIZE)baseVirAddr + offset);

	ACPI_DEBUG_PRINT ((ACPI_DB_EXEC, "AcpiOsMapMemory %08X %d => %08X - %08X %d => %08X\n", Where, Length, fromPage, toPage, sizePage, virAddr));
    ...
    ACPI_SIZE               Length)
{
    ...
    ACPI_FUNCTION_NAME (AcpiOsUnmapMemory);

    b = NKDeleteStaticMapping((void*)fromPage, sizePage);

	ACPI_DEBUG_PRINT ((ACPI_DB_EXEC, "AcpiOsUnmapMemory (%d) %08X %d => %08X - %08X %d\n", b, Where, Length, fromPage, toPage, sizePage));
...
// pgx start
    ACPI_FUNCTION_NAME (AcpiOsAllocate);

    //Mem = (void *) malloc ((size_t) Size);
    Mem = os_malloc((size_t) Size);
    ACPI_DEBUG_PRINT ((ACPI_DB_ALLOCATIONS, "%08X %d\n", (UINT32)Mem, Size));
//pgx end
...
{
// pgx start
    ACPI_FUNCTION_NAME (AcpiOsFree);

    //free (Mem);
    os_free (Mem);
    ACPI_DEBUG_PRINT ((ACPI_DB_ALLOCATIONS, "%08X\n", (UINT32)Mem));
// pgx end
}
...
// pgx start
    ...
    //*Value = 0;
    x = PCIReadBusData(PciId->Bus, PciId->Device, PciId->Function, Value, Register, Width / 8);
	ACPI_DEBUG_PRINT ((ACPI_DB_EXEC, "PCIReadBusData %d %d %d %02X %d", PciId->Bus, PciId->Device, PciId->Function, Register, x));
// pgx end
    return (AE_OK);
}
...
// pgx start
    ...
    x = PCIWriteBusData(PciId->Bus, PciId->Device, PciId->Function, &Value, Register, Width / 8);
	ACPI_DEBUG_PRINT ((ACPI_DB_EXEC, "PCIWriteBusData %d %d %d %02X %d", PciId->Bus, PciId->Device, PciId->Function, Register, x));
// pgx end
    return (AE_OK);
}
