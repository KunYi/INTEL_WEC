...
#define MAX_CD_SECT_PER_COMMAND           512     // Changing this to be 1MB for 2048-byte sectors.
...
#define REG_VAL_IDE_LEGACYIRQ             (_T("LegacyIRQ"))             // IRQ; IRQ of primary legacy channel i.e., primary 14 (will cause secondary to use 15)
#define REG_VAL_IDE_IRQ                   (_T("Irq"))                   // IDE/ATA channel's IRQ
#define REG_VAL_IDE_SYSINTR               (_T("SysIntr"))               // IDE/ATA channel's SysIntr
#define REG_VAL_IDE_VENDORID              (_T("VendorID"))              // vendor id; identify controller; needed by PCI Bus
#define REG_VAL_IDE_DEVICEID              (_T("DeviceID"))              // Device id; identify controller; needed by PCI Bus
...
#define REG_VAL_IDE_DRIVERCONSTRUCTOR     (_T("DriverConstructor"))     // name of function to create controller-specific CIDEBUS instance
...
#define REG_VAL_DMA_TRANSFERMODE     (_T("DMATransferMode"))  // 1 byte transfer mode that is DMA specific
#define REG_VAL_PIO_TRANSFERMODE     (_T("PIOTransferMode"))  // 1 byte transfer mode that is PIO specific
...
#ifdef I830M4
    DDKPCIINFO     m_ppi;                                  // PCI information for the controller
    WORD           m_wMaxPMState;                          // The most device power state supported
#endif
...
//    CPort         *m_pSecondaryPort;                       // I/O port of secondary channel
    ...
    DWORD          m_dwControllerBaseStatic;               // Controller's static base address for use by the interrupt handler
...
class CIDEController : public CIDEBUS
{
public:
    virtual BOOL Init(PIDEREG p_idereg) = 0;
    virtual BOOL MapIORegisters(HKEY hDevKey, PTSTR szDevKey) = 0;
    virtual BOOL MapIrqtoSysIntr() = 0;
    virtual DWORD GetNumPorts() = 0;
    virtual CPort * GetPort(unsigned int i) = 0;
    virtual BOOL IsMasterSlaveDevice() = 0;
};

extern "C" CIDEController * NewCATAController();

// To indicate whether master device is deleted.
extern BOOL    bIsNativeMasterDeviceDeleted;
// To indicate Bus Master Controller is generating the interrupt
extern BOOL    bIsBMIRQ;
