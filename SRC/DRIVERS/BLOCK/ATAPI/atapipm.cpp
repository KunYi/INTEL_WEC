...
#pragma warning(push)          // fSuccess is only used in DEBUG builds
...
BOOL CDiskPower::Init(CDisk *pDiskParent)
{
    ...
    if(fOk) {
        ...
        if(fOk) {
            ...
            DWORD dwChars = 0;
            LPWSTR pszPMName = NULL;
            if( SUCCEEDED(StringCchLength( szName, _countof(szName), (size_t*)&dwChars )) )
            {
                dwChars += _countof(PMCLASS_BLOCK_DEVICE) + 1;  // class + separator + name + null
                pszPMName = (LPWSTR) LocalAlloc(LPTR, dwChars * sizeof(WCHAR));
