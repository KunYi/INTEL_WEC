...
//     read.
// ----------------------------------------------------------------------------
BOOL CSgReq::DoReadMultiple(__out_bcount(dwBufSize) BYTE* pbBuf, DWORD dwBufSize, DWORD dwBytes, PDWORD pdwBytesRead)
{
    DWORD dwBytesLeftInCurrentBuffer = 0;
    ...
    {
        ASSERT(!"Buffer is too small for the requested read size");
        return FALSE;
    }
...
//     written.
// ----------------------------------------------------------------------------
BOOL CSgReq::DoWriteMultiple( __in_bcount(dwBytes) BYTE* pbBuf, DWORD dwBytes, PDWORD pdwBytesWritten)
{
    DWORD dwBytesLeftInCurrentBuffer = 0;
