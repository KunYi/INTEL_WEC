...
class CSgReq {
  ...
  public:
    ...
    BOOL         DoReadMultiple(PBYTE pbBuf, DWORD dwBufSize, DWORD dwBytes, PDWORD pdwBytesRead);
...
{
    DWORD dwBytesRead = 0;
    if (FALSE == this->DoReadMultiple(pb, 1, 1, &dwBytesRead)) {
        return FALSE;
    }
    if (1 != dwBytesRead) {
        return FALSE;
...
{
    DWORD dwBytesRead = 0;
    if (FALSE == this->DoReadMultiple((PBYTE)pw, 2, 2, &dwBytesRead)) {
        return FALSE;
    }
    if (2 != dwBytesRead) {
        return FALSE;
