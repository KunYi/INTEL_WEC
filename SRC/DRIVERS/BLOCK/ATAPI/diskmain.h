...
#define ATA_READ_BYTE(p) ((m_pPort->m_pController->m_bisIOMapped)?READ_PORT_UCHAR(p):READ_REGISTER_UCHAR(p))
#define ATA_READ_WORD(p) ((m_pPort->m_pController->m_bisIOMapped)?READ_PORT_USHORT(p):READ_REGISTER_USHORT(p))
#define ATA_READ_DWORD(p) ((m_pPort->m_pController->m_bisIOMapped)?READ_PORT_ULONG(p):READ_REGISTER_ULONG(p))
#define ATA_WRITE_BYTE(p,b) ((m_pPort->m_pController->m_bisIOMapped)?WRITE_PORT_UCHAR((p),(b)):WRITE_REGISTER_UCHAR((p),(b)))
#define ATA_WRITE_WORD(p,w) ((m_pPort->m_pController->m_bisIOMapped)?WRITE_PORT_USHORT((p),(w)):WRITE_REGISTER_USHORT((p),(w)))
#define ATA_WRITE_DWORD(p,dw) ((m_pPort->m_pController->m_bisIOMapped)?WRITE_PORT_ULONG((p),(dw)):WRITE_REGISTER_ULONG((p),(dw)))
...
class CDisk {
  ...
  public:
    ...
    volatile PBYTE m_pSATAReg;       // Base address of SATA register set (MmMapped)
...
public:
    ...
    inline void ReadByteBuffer(__inout_ecount(dwCount) PBYTE pBuffer, DWORD dwCount) {
    ...
    inline void ReadWordBuffer(__inout_ecount(dwCount) PWORD pBuffer, DWORD dwCount) {
    ...
    inline void WriteByteBuffer(const BYTE* pBuffer, DWORD dwCount) {
        volatile BYTE * const Port = (volatile BYTE * const)(m_pATAReg+m_dwDataDrvCtrlOffset);
        if (m_pPort->m_pController->m_bisIOMapped)
    ...
    inline void WriteWordBuffer(const WORD* pBuffer, DWORD dwCount) {
        volatile USHORT * const Port = (volatile USHORT * const)(m_pATAReg+m_dwDataDrvCtrlOffset);
        if (m_pPort->m_pController->m_bisIOMapped)
    ...
    void SetActiveKey(const TCHAR *szActiveKey);
    // Called by DSK_Init; this information is used by CDiskPower
    void SetDeviceKey(const TCHAR *szDeviceKey);
    ...
    BOOL static ValidateSg(PSG_REQ pSgReq, DWORD InBufLen, DWORD dwArgType, PUCHAR * saveoldptrs, DWORD dwOldPtrCount);
