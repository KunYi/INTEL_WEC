    ...
    )
{
    ...
    pcdb->Len = (USHORT)(((sizeof (RKFMT_DISC)& 0xFF) << 8) | (sizeof (RKFMT_DISC) >>8));
    pcdb->agid = bAgid;
    pcdb->NACA = 0;
    ...
    )
{
    ...
    if (AtapiSendCommand((CDB*)&CmdPkt)) {
...
/*
    ...
    Return
        ...
        the target drive is a never-used drive, i.e., its RPC Setting (Region) has
    ...
    Notes
        ...
        -------------------
        00b | NONE; No drive region setting 
    ...
    if (AtapiSendCommand((CDB*)&CmdPkt)) {
        ...
        if (AtapiReceiveData(&SgBuf, 1, &dwRet)) {
            ...
            if (rpcData.ResetCounts & 0xc0) {
                ...
                // the drive is in NONE, i.e., this drive has not been used
                DEBUGMSG(ZONE_ERROR|ZONE_CDROM, (_T(
