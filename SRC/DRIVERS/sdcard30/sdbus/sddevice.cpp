...
VOID CSDDevice::UpdateCachedRegisterFromResponse(SD_INFO_TYPE  Register,PSD_COMMAND_RESPONSE     pResponse)
{
    ...
    switch (Register) {
        ...
        case SD_INFO_REGISTER_CID:
            RetVal = memcpy_s(m_CachedRegisters.CID, SD_CID_REGISTER_SIZE, pResponse->ResponseBuffer, SD_CID_REGISTER_SIZE);
            ASSERT(RetVal == 0 );
            break;
        ...
        case SD_INFO_REGISTER_CSD:
            RetVal = memcpy_s(m_CachedRegisters.CSD, SD_CSD_REGISTER_SIZE, pResponse->ResponseBuffer, SD_CSD_REGISTER_SIZE);
            ASSERT(RetVal == 0 );
            break;
        ...
        if (dwSDSpecVersion >=1 ) { // Spec 1.1 and above.
            if (fReadOnly) {
                ...
                if (SD_API_SUCCESS(status)) {
                    ...
                    RetVal = memcpy_s(pSwitchData->clientData, sizeof(pSwitchData->clientData), SwitchStatus, sizeof(SwitchStatus));
                    ASSERT(RetVal == 0);
                }
            }
            else {
