                             ...
                             )
{
    ...
    PCSDHCSlotBase pSlot = GetSlot(dwSlot);
#ifdef DEBUG
    if (sdOption >= _countof(sc_rgpszOptions)) {
        ASSERT(sdOption < _countof(sc_rgpszOptions));
...
CSDHCBase::IST()
{
    ...
    for (;;) {
        ...
        if (WAIT_OBJECT_0 != dwWaitStatus) {
            ...
            DEBUGMSG(SDHC_INTERRUPT_ZONE, (TEXT("%s Wait timeout Slot Interrupt Reg x%08X ! Wait status 0x%08X\n"),pszFname,pSlotZero->ReadControllerInterrupts(), dwWaitStatus));
            DBG_UNREFERENCED_LOCAL_VARIABLE(pSlotZero);
        }
        else {
            HandleInterrupt();
